...
from utils.util import _triple
...
def grid_sample_with_kernel_offsets(input, offsets):
  # TODO: other kernel shapes
  grid = construct_3d_kernel_grid(input.shape[-3:], device=input.device)
  offsets = einops.rearrange(offsets, "n (K c) d h w -> n K c d h w", c=3, K=27)
  grid = grid + offsets
  grid = einops.rearrange(grid, "n K c d h w -> n (K d) h w c", c=3)
  sampled_points = F.grid_sample(input.to(grid.dtype), grid)
  return einops.rearrange(sampled_points, "n c (K d) h w -> n (K c) d h w", K=27)


def offsets_from_size_map(size_map, mode, kernel_shape=(3, 3, 3), dilation=(1, 1, 1)):
  ...
  mode = mode.lower()
  if mode == "xyz":
    assert size_map.shape[1] == 3
    # no-op, size_map is already 3-dimensional
    pass
  elif mode == "xxz":
    assert size_map.shape[1] == 2
    z, x = torch.split(size_map, [1, 1], dim=1)
    size_map = torch.cat([z, x, x], dim=1)
  elif mode == "xy":
    assert size_map.shape[1] == 2
    size_map = torch.cat([ones, size_map], dim=1)
  elif mode == "x":
    assert size_map.shape[1] == 1
...
def generate_offsets_from_size_and_flow(size_map, flow_map):
  kernel_shape = (3, 3, 3)
  dilation = (1, 1, 1)
  base_offset = get_kernel_grid(kernel_shape, dilation, size_map.device)
  base_offset = einops.rearrange(base_offset, "zyx kz ky kx -> kz ky kx zyx 1 1 1", zyx=3)

  B, C, D, H, W = size_map.shape
  assert C == 2 or C == 1, f"expected 1 or 2 dimensions for size map, not {C}"
  ones = torch.ones(B, 1, D, H, W, device=size_map.device)
  if C == 1:
    size_map = torch.cat([ones, size_map, size_map], dim=1)
  else:
    size_map = torch.cat([ones, size_map], dim=1)

  B, C, D, H, W = flow_map.shape
  assert C == 4 or C == 6, f"expected 4 or 6 dimensions for flow map, not {C}"
  flow_next, flow_prev = torch.split(flow_map, [C//2, C//2], dim=1)
  if C == 4:
    flow_next = torch.cat([ones, flow_next], dim=1)
    flow_prev = torch.cat([ones, flow_prev], dim=1)

  kernel_grid = construct_3d_kernel_grid((D, H, W), kernel_size=(1, 3, 3), pad=(0, 1, 1), device=flow_map.device)
  sample_grid_next = einops.rearrange(kernel_grid + flow_next[:, None, ...], "b K zyx d h w -> b (K d) h w zyx")
  sample_grid_prev = einops.rearrange(kernel_grid + flow_prev[:, None, ...], "b K zyx d h w -> b (K d) h w zyx")
  sizes_next = F.grid_sample(size_map, sample_grid_next, padding_mode="border", align_corners=False)
  sizes_prev = F.grid_sample(size_map, sample_grid_prev, padding_mode="border", align_corners=False)

  sizes_next = einops.rearrange(sizes_next, "b zyx (Ky Kx d) h w -> b 1 Ky Kx zyx d h w", Ky=3, Kx=3)
  sizes_this = einops.repeat(size_map, "b zyx d h w -> b 1 Ky Kx zyx d h w", Ky=3, Kx=3)
  sizes_prev = einops.rearrange(sizes_prev, "b zyx (Ky Kx d) h w -> b 1 Ky Kx zyx d h w", Ky=3, Kx=3)
  flow_next = einops.rearrange(flow_next, "b zyx d h w -> b 1 1 1 zyx d h w")
  flow_prev = einops.rearrange(flow_prev, "b zyx d h w -> b 1 1 1 zyx d h w")

  offset_next = base_offset[0] * sizes_next + flow_next
  offset_this = base_offset[1] * sizes_this
  offset_prev = base_offset[2] * sizes_prev + flow_prev
  offsets = torch.cat([offset_prev, offset_this, offset_next], dim=1)
  offsets = offsets - base_offset

  return einops.rearrange(offsets, "b Kz Ky Kx zyx d h w -> b (Kz Ky Kx zyx) d h w", zyx=3)
